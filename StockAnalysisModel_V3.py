...
.spinner {
  ...
  border:3px solid #ccc; border-top-color:#1f77b4; border-radius:50%;
  animation: spin 0.9s linear infinite; vertical-align:middle; margin-right:8px;
}
.msg { display:inline-block; vertical-align:middle; font-weight:600; }
.hint { color:#666; font-size:12px; margin-top:6px; }
</style>
<div><span class="spinner"></span><span class="msg">Calculating…</span></div>
<div class="hint">This may take 10–30 seconds depending on data and network.</div>
""")

def get_stock_data(ticker, period='max', retries=5):
    ...
    for attempt in range(retries):
        try:
            ...
            hist = hist.loc[hist.index >= (hist.index.max() - pd.DateOffset(years=12))]
...
# Replace your current train_model() with this
def train_model(df):
    """Train RandomForest with chronological split; guard against tiny datasets."""
    from sklearn.ensemble import RandomForestRegressor
    from sklearn.preprocessing import StandardScaler

    # Minimum rows for a meaningful split (tune as you like)
    MIN_ROWS = 80  # ~2-3 months of valid rows after dropna for daily data
    if len(df) < MIN_ROWS:
        raise RuntimeError(f"Too few samples to train the ML model (n={len(df)} < {MIN_ROWS}).")

    X = df.drop(['Future_Return', 'Close'], axis=1)
    y = df['Future_Return']

    # Chronological split with guaranteed non-empty train & test
    split_idx = max(1, int(len(df) * 0.8))
    if split_idx >= len(df):
        split_idx = len(df) - 1  # ensure at least 1 sample in test
    ...
    r2_clamped = float(np.clip(r2, 0.0, 1.0))  # keep [0,1] for scoring
    return model, scaler, r2_clamped
...
def evaluate_investment(ticker, horizon_months=12, period ='max'):
    """Evaluate the company with Morningstar-inspired presentation."""
    hist, fundamentals, ticker = get_stock_data(ticker, period=period)
...
def on_analyze_button_clicked(b):
    ...
    analyze_button.description = 'Calculating…'
    try:
        with output:
            clear_output(wait=True)   # ensures spinner shows immediately
            display(spinner)
        ...
        analyze_button.description = 'Analyze Stock'
